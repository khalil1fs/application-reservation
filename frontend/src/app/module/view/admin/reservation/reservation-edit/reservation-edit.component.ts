...
export class ReservationEditComponent implements OnInit {
 ...
 ngOnInit(): void {
  ...
  edit() {
    this.reservationService.edit().subscribe(
      data => { 
  ...
  private prepare(data: Reservation) {
    let message = 'Reservation edited successfully';
    let messageType = 'snackbar-success';
    if (data.id == null) {
      message = 'Error while editing reservation or Reference already exist';
      messageType = 'snackbar-danger';
    } else
    {
      this.reservations = this.reservations.filter(i => i.id != data.id);
